...
public class AudioTag extends HashMap<AudioTag.Attribute, String> {
	...
	public static AudioTag read(File f) {
		...
		if (abstractF instanceof MP3File)
	...
	private static AudioTag readGeneric(AudioFile f) {
		...
		while (fields.hasNext()) {
			...
			if (tf instanceof TagTextField) {
				...
				String tagBody = ((TagTextField)tf).getContent();
				if (at.containsKey(a))
	...
	private static AudioTag readMP3(MP3File f) {
		...
		MP3AudioHeader header = f.getMP3AudioHeader();
		at.put(Attribute.BITRATE, header.getBitRate().startsWith("~") ? header.getBitRate().substring(1) : header.getBitRate());
		at.put(Attribute.DURATION, "" + (int)(header.getPreciseTrackLength() * 1000));
		...
		String TDAT = null;
		while (fields.hasNext()) {
			AbstractID3v2Frame v2Frame = (AbstractID3v2Frame) fields.next();
			// AbstractFrameBodyTextInfo textBody = (AbstractFrameBodyTextInfo) v2Frame.getBody();
			String tagID = v2Frame.getBody().getIdentifier();
			if (v2Frame.getBody() instanceof FrameBodyTXXX)
				tagID = "TXXX:" + ((FrameBodyTXXX)v2Frame.getBody()).getDescription();
			String tagBody = v2Frame.getBody().getUserFriendlyValue();
			...
			if (a == Attribute.DATE && TDAT != null)
				at.put(a, tagBody.trim() + "-" + TDAT.substring(2, 4) + "-" + TDAT.substring(0, 2));
			else if (a != null)
				at.put(a, tagBody.trim());
		}
	...
	private static Attribute getAttributeByTagIdentifier(String tagID) {
		...
		if ((tagID.equalsIgnoreCase("MUSICBRAINZ_RECORDING_ID")) || (tagID.equalsIgnoreCase("UFID:http://musicbrainz.org")) || (tagID.equalsIgnoreCase("----:com.apple.iTunes:MusicBrainz Track Id")) || (tagID.equalsIgnoreCase("MUSICBRAINZ_TRACKID")) || (tagID.equalsIgnoreCase("MusicBrainz/Track Id")))
